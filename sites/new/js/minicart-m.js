...
(function () {
	...
	PAYPAL.apps.MiniCart = (function () {
		...
		var _addCSS = function () {
			...
			css.push('#' + name + ' form { position:relative; width:100%; margin:0; padding:55px 0 0 0; background:#fff url(' + config.assetURL + 'images/minicart_sprite.png) no-repeat -125px -60px; } ');
			css.push('#' + name + ' ul { position:relative; list-style:none; margin: 0 0 10px 0; padding: 0; border-bottom: 1px solid #ddd;} ');
			css.push('#' + name + ' li { position:relative; margin: 0; padding: 10px; border-top:1px solid #ddd; } ');
			css.push('#' + name + ' li a { color:#333; text-decoration:none; } ');
			css.push('#' + name + ' li a span { color:#999; font-size:10px; } ');
			css.push('#' + name + ' li .quantity { position:absolute; top:.5em; right:105px; width:22px; padding:1px; border:1px solid #83a8cc; text-align:right; } ');
			css.push('#' + name + ' li .price { position:absolute; top:.5em; right:10px; } ');
			css.push('#' + name + ' li .remove { position:absolute; top:12px; right:70px; width:14px; height:14px; background:url(' + config.assetURL + 'images/minicart_sprite.png) no-repeat -134px -4px; border:0; cursor:pointer; } ');
			css.push('#' + name + ' p { margin:0; padding:0 10px 10px 10px; font-size:13px; font-weight:bold; } ');
		...
		minicart.render = function (userConfig) {
			...
			if (isShowing) {
				setTimeout(function () {
				  var subtotal = minicart.UI.subtotalAmount.innerHTML;	
				  if(subtotal=='0.00'){	
					  minicart.hide(null);
				  }
				}, 500);
