...
class MenuItem{
	...
	// Updates the url
	public static function UpdateUrl($pageId, $url){
		...
		$result = mysql_query(
			"UPDATE MenuItems SET Url = '$url' WHERE PageId = $pageId");
	
		if(!$result) {
		  print "Could not successfully run query MenuItem->UpdateUrl" . mysql_error() . "<br>";
		  exit;
		}
	...
	public static function DeleteAll($siteId, $type){
		
		Connect::init();
		
		settype($siteId, 'integer');
	
		$delete = mysql_query("DELETE FROM MenuItems WHERE SiteId='$siteId' AND Type='$type'");
